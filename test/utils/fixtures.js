 ...
 *
 * This repo's fixtures are not grouped into per-category directories (unlike
 * chartjs-chart-treemap, where this helper originates): most live directly
 * under `test/fixtures`, with only `issues/` as a subdirectory. An empty
 * `path` therefore means "every fixture", matching every name rather than
 * only those below a `${path}/` prefix.
 * @param {string} [path] - directory under `test/fixtures`, e.g. `issues`; omit for all fixtures
 */
