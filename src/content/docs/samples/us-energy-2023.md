  ...
  config: {
      ...
      datasets: [
        {
          ...
          colorFrom: (context) => colors[context.dataset.data[context.dataIndex].from],
          colorTo: (context) => colors[context.dataset.data[context.dataIndex].to],
          linkColor: (context) => {
            const { from, to } = context.dataset.data[context.dataIndex]
    ...
    options: {
      plugins: {
        tooltip: {
          callbacks: {
            label: (context) => `${context.raw.from} → ${context.raw.to}: ${context.raw.flow} quads`,
          },
        },
      },
    },
